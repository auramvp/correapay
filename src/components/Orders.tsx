...
export default function Orders({ orders, clients, onAdd, onUpdate, onDelete }: OrdersProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const orderData = {
      ...
      receiptUrl: receiptFile ? URL.createObjectURL(receiptFile) : (editingOrder?.receiptUrl || undefined)
    };
    ...
    if (editingOrder) {
      ...
      // Clear form but keep modal open for "novo deposito" as per user request
      setAmountStr('');
      setFixedFeeStr('');
      setRecipient('');
      setBank('');
                ...
                <div className="pt-4">
                  {viewingOrder.receiptUrl ? (
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button 
                          onClick={() => viewingOrder.receiptUrl && window.open(viewingOrder.receiptUrl, '_blank')}
